...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.x_0_pred_coef_1 = torch.sqrt(1/self.alphas_cumprod)
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        batched_tau_isub1 = torch.full((img.shape[0],), tau_isub1, device=self.device, dtype=torch.long).clip(min=0)

        pred_noise_tau_i, x_0 = self.model_predictions(img, batched_tau_i)

        # Step 2: Extract \alpha_{\tau_{i - 1}} and \alpha_{\tau_{i}}
        alpha_tau_i = extract(self.alphas_cumprod, batched_tau_i, img.shape)
        alpha_tau_isub1 = extract(self.alphas_cumprod, batched_tau_isub1, img.shape)

        # Step 3: Compute \sigma_{\tau_{i}}
        beta_tau_i = extract(self.betas, batched_tau_i, img.shape)
        sigma_tau_i = torch.sqrt(((1 - alpha_tau_isub1) * beta_tau_i) / (1 - alpha_tau_i))

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        coef_epsilon_tau_i = torch.sqrt(1. - alpha_tau_isub1 - sigma_tau_i**2)

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        z = torch.randn(img.shape).to(self.device)
        img = (torch.sqrt(alpha_tau_isub1)*x_0) + (coef_epsilon_tau_i*pred_noise_tau_i) + (sigma_tau_i*z)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
